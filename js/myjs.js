...
// ф-н для смены языка+значёк #1
//function changeLang() {
//
//	//	console.log(document.getElementById("lang").innerHTML);
//	if (count == 0) {
//		document.getElementById("lang").innerHTML = engIcon;
//		count = 1;
//	} else {
//		document.getElementById("lang").innerHTML = rusIcon;
//		count = 0;
//	}
//}

//f-n for change language #2
function changeLang() {
	$("#lang_icon").toggleClass("langRu");
	if (count==0){
		$("#lang_text").text("ENG");
		count=1;
	}
...
// f-n for show search area
function showInput() {
	//	if ( $("#search_show").show )
	if ($("#input_search").is(':hidden')) {
		...
		$("#input_search").toggleClass("input_animation");
	}
}

function showAddText() {
	$("#hiddenText").slideToggle('slow');
	if ($("#addText").text() == "Подробно") {
		$("#addText").text("Скрыть")
	} else {
		$("#addText").text("Подробно");
	}
}

// binding for click
$("#lang").click(changeLang); // this jQuery use
$("#search_show").click(showInput);
$("#addText").click(showAddText);
